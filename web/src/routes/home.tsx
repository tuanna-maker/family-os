...
import { getFamilyToday, type FamilyTodayMember, type StatusTone } from "@/lib/family-today.functions";
import { getSecurityStatus, type SecurityChip, type SecurityTone } from "@/lib/security.functions";
...
function HomePage() {
  ...
  const activitiesQuery = useQuery({
    ...
    queryFn: () => fetchNotifications({ data: { limit, offset: 0 } }),
  });
  const fetchSecurity = useServerFn(getSecurityStatus);
  const familyTodayQuery = useQuery({
    queryKey: ["family-today", familyId],
    queryFn: () => fetchFamilyToday({ data: { family_id: familyId! } }),
    enabled: !!familyId,
  });
  const securityQuery = useQuery({
    queryKey: ["security-status", familyId],
    queryFn: () => fetchSecurity({ data: { family_id: familyId! } }),
    enabled: !!familyId,
