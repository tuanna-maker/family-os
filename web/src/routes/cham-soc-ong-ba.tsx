...
} from "@/lib/elderly-care.functions";
...
import type { MedicineReminderRow } from "@/lib/elderly-care.functions";
...
function ElderlyCarePage() {
  ...
  // ============ QUERIES ============
  const profilesFn = useServerFn(listElderlyProfiles);
  ...
  const medsFn = useServerFn(listMedicineReminders);
  ...
  const notesFn = useServerFn(listCareNotes);
  ...
  const actQ = useQuery({
    queryKey: ["elderly-activity", profile?.id, profile?.name],
    queryFn: () =>
      actFn({ data: { elderlyId: profile!.id, familyId: familyId!, memberName: profile!.name } }),
  ...
  const createProfile = useMutation({
    mutationFn: useServerFn(createElderlyProfile),
    onSuccess: () => {
  ...
  const confirmSafeFn = useServerFn(confirmSafeCheck);
  ...
  const takenMut = useMutation({
    mutationFn: useServerFn(markMedicineTaken),
    onSuccess: () => {
      qc.invalidateQueries({ queryKey: ["elderly-meds", familyId, profile?.name] });
      qc.invalidateQueries({ queryKey: ["elderly-meds-week", familyId, profile?.name] });
    },
  ...
  const undoMut = useMutation({
    mutationFn: useServerFn(undoMedicineTaken),
    onSuccess: () => {
      qc.invalidateQueries({ queryKey: ["elderly-meds", familyId, profile?.name] });
      qc.invalidateQueries({ queryKey: ["elderly-meds-week", familyId, profile?.name] });
    },
  ...
  const createMed = useMutation({
    mutationFn: useServerFn(createMedicineReminder),
    onSuccess: () => {
  ...
  const addNoteMut = useMutation({
    mutationFn: useServerFn(addCareNote),
    onSuccess: () => {
  ...
  const sendSecurityRequest = useServerFn(createSecurityRequest);
  const qc2 = qc;
  ...
  async function handleCall(label: string, phone: string, isSos = false) {
    if (isSos) {
      if (session) {
        try {
          await sendSecurityRequest({ data: { request_type: "sos" } });
  ...
  return (
    <MobileShell>
      ...
      {famLoading || profilesQ.isLoading ? (
        ...
        <EmptyProfilesCard
          onAdd={(p) =>
            createProfile.mutate({ data: { family_id: familyId!, ...p } })
          }
                    ...
                    {meds.map((m) => (
                      <div key={m.id} className="flex items-center gap-3 p-4">
                        ...
                        <button
                          ...
                          onClick={() => {
                            if (m.taken_today) {
                              undoMut.mutate({ data: { reminder_id: m.id } });
                            } else {
                              const now = new Date();
                              const hh = String(now.getHours()).padStart(2, "0");
                              const mm = String(now.getMinutes()).padStart(2, "0");
