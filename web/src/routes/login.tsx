...
import { resolveLoginEmail, checkUsernameAvailable } from "@/lib/username.functions";
import { resolveDestinationPure } from "@/lib/resolve-destination";
import { listPublicProjects } from "@/lib/projects-public.functions";
import {
  Loader2,
  Mail,
  Lock,
...
function LoginPage() {
  ...
  const fetchProjects = useServerFn(listPublicProjects);
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        ...
        if (!cancelled) await navigate({ to, replace: true });
      } catch (err) {
        if (!cancelled) {
          console.error("[login] workspace redirect failed", err);
          setError("Chưa mở được workspace. Vui lòng thử lại hoặc chọn workspace thủ công.");
        }
      }
    })().finally(() => {
    ...
    try {
      if (mode === "signin") {
        ...
        if (!loginEmail.includes("@")) {
          ...
          const res = await resolveLoginEmail({ data: { username: loginEmail } });
        ...
        const check = await checkUsernameAvailable({ data: { username: username.trim() } });
        if (!check.available) {
    ...
    <div className="min-h-screen flex bg-background relative">
      ...
      <div className="flex-1 flex items-center justify-center px-4 py-10 sm:px-8">
        <div className="w-full max-w-md">
          <Link
            to="/"
            className="hide-in-pwa inline-flex items-center gap-1.5 text-sm text-muted-foreground hover:text-foreground mb-6"
          >
