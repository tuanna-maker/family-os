...
import { getMyPrefs, updateMyPrefs, listFamilyPrefs, type NotificationPrefs } from "@/lib/notification-prefs.functions";
import { useEasyRead } from "@/hooks/use-easy-read";
...
function NotifSettingsPage() {
  const get = useServerFn(getMyPrefs);
  const list = useServerFn(listFamilyPrefs);

  const me = useQuery({ queryKey: ["np", "me"], queryFn: () => get() });
  const fam = useQuery({ queryKey: ["np", "family"], queryFn: () => list() });
...
function PrefForm({ initial }: { initial: NotificationPrefs; ownRow?: boolean }) {
  ...
  const update = useServerFn(updateMyPrefs);
