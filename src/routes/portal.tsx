...
import { suggestMeals } from "@/lib/food.functions";
import { listFamilyEvents, type FamilyEventRow } from "@/lib/family-events.functions";
...
function PortalPage() {
  ...
  const { data: d } = useQuery({
    ...
    queryFn: () => dash({ data: { family_id: familyId! } }),
    enabled: !!familyId,
    refetchInterval: 60_000,
  });

  const meals = useServerFn(suggestMeals);
  const { data: foodData } = useQuery({
    queryKey: ["meals", familyId],
    queryFn: () => meals({ data: { family_id: familyId! } }),
    enabled: !!familyId,
  ...
  const events = useServerFn(listFamilyEvents);
  const todayRange = (() => {
  ...
  const { data: todayEvents } = useQuery({
    ...
    queryFn: () => events({ data: { family_id: familyId!, from: todayRange.from, to: todayRange.to } }),
    enabled: !!familyId,
  ...
  const { data: notifData } = useQuery({
    ...
    queryFn: () => notifs({ data: { only_unread: true, limit: 6 } }),
    refetchInterval: 60_000,
  });
