...
function FoodPage() {
  ...
  const list = useServerFn(listFood);
  const suggest = useServerFn(suggestMeals);
  const tgShop = useServerFn(toggleShopping);
  const del = useServerFn(deleteFoodRow);
  ...
  const mealsQ = useQuery({
    ...
    queryFn: () => suggest({ data: { family_id: familyId! } }),
    enabled: !!familyId,
  });
  ...
  const tgMut = useMutation({
    mutationFn: (v: { id: string; purchased: boolean }) => tgShop({ data: v }),
...
function FoodDialog({ state, familyId, onClose, onSaved }: { state: NonNullable<Dlg>; familyId: string; onClose: () => void; onSaved: () => void }) {
  const saveFood = useServerFn(upsertFoodItem);
  const saveShop = useServerFn(upsertShoppingItem);
  ...
  const mut = useMutation({
    mutationFn: async () => {
      ...
      if (state.type === "food") {
        return saveFood({ data: { ...base, qty: form.qty ? Number(form.qty) : null } });
      }
      return saveShop({ data: { ...base, qty: form.qty ? Number(form.qty) : null, purchased: form.purchased ?? false } });
    },
