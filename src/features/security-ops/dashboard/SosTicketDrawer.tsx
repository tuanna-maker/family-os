...
export function SosTicketDrawer({
  ...
  const statusMut = useMutation({
    mutationFn: (vars: { status: SosStatus; note?: string }) =>
      updateFn({ data: { id: row!.id, status: vars.status, note: vars.note } }),
    onSuccess: (_d, vars) => {
  ...
  const noteMut = useMutation({
    mutationFn: (n: string) => noteFn({ data: { id: row!.id, note: n } }),
    onSuccess: () => {
