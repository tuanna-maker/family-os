...
export function AuthProvider({ children }: { children: ReactNode }) {
  ...
  useEffect(() => {
    ...
    const { data: { subscription } } = supabase.auth.onAuthStateChange((event, s) => {
      ...
      const shouldInvalidate = changed && !(event === "SIGNED_IN" && isLoginRoute);
      log(event, s, { prevUserId, nextUserId, willInvalidate: shouldInvalidate });
      if (shouldInvalidate) {
        router.invalidate();
  ...
  return (
    <Ctx.Provider
      value={{
        ...
        loading,
        signOut: async () => {
          // Xoá session ở mọi tab + revoke refresh token phía server
          try {
            await supabase.auth.signOut({ scope: "global" });
          } catch {
            // fallback: ít nhất xoá session local
            try { await supabase.auth.signOut({ scope: "local" }); } catch { /* ignore */ }
          }
          // Quét sạch token Supabase còn sót trong storage (đề phòng key cũ)
          ...
          // Reset state + cache để lần đăng nhập sau không dính dữ liệu user cũ
          setSession(null);
          qc.clear();
          router.invalidate();
        },
