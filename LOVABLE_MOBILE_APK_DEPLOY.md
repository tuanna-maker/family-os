...
### Bước 4 — Web (deploy code mới)
QR phải trỏ **thẳng Supabase URL** (cross-origin), KHÔNG dùng link cùng domain `stoslife.lovable.app`:
- PWA `manifest-family.json` có `scope: "/"` → quét QR cùng domain sẽ mở `/login?source=pwa-family` thay vì tải APK.

Sau deploy, QR encode:
- `https://bigarvjahnxiuovepaxm.supabase.co/storage/v1/object/public/mobile-apks/stos-guard.apk`
- `https://bigarvjahnxiuovepaxm.supabase.co/storage/v1/object/public/mobile-apks/stos-family.apk`
