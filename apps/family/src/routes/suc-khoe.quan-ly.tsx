...
function HealthPage() {
  ...
  const delMut = useMutation({
    mutationFn: (v: { table: any; id: string }) => deleteHealthRow(v),
    onSuccess: () => {
...
function HealthDialog({
  ...
  const mut = useMutation({
    mutationFn: async () => {
      ...
      if (state.type === "profile") return upsertHealthProfile(base);
      if (state.type === "medicine") return upsertMedicine({ ...base, active: form.active ?? true });
      if (state.type === "appt") {
        if (!form.scheduled_at) throw new Error("Vui lòng chọn thời gian");
        return upsertAppointment({ ...base, scheduled_at: new Date(form.scheduled_at).toISOString(), status: form.status ?? "planned" });
      }
      return upsertHealthRecord({ ...base, recorded_at: form.recorded_at ? new Date(form.recorded_at).toISOString() : undefined });
    },
