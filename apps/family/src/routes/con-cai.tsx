...
function ChildrenDialog({
  ...
  const mut = useMutation({
    mutationFn: async () => {
      ...
      if (state.type === "child") return upsertChild(base);
      if (state.type === "schedule") return upsertSchedule({ ...base, day_of_week: Number(form.day_of_week ?? 1) });
      if (state.type === "homework") return upsertHomework({ ...base, done: form.done ?? false });
      if (state.type === "achievement") return upsertAchievement(base);
      if (state.type === "reminder") {
        if (!form.remind_at) throw new Error("Chọn thời gian");
        return upsertParentReminder({ ...base, remind_at: new Date(form.remind_at).toISOString(), done: form.done ?? false });
      }
    },
