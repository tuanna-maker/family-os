-- Inbox: avatar cư dân + bỏ tin auto-reply khỏi preview cuối.

CREATE OR REPLACE FUNCTION public.is_legacy_security_chat_auto_reply(_body text, _sender_role text)
RETURNS boolean
LANGUAGE sql
IMMUTABLE
AS $$
  SELECT _sender_role = 'guard'
    AND (
      COALESCE(_body, '') ~* 'Bảo an đã nhận tin'
      OR COALESCE(_body, '') ~* 'Security received your message'
    );
$$;

DROP FUNCTION IF EXISTS public.list_guard_security_chat_inbox();
