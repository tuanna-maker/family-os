...
export function SecurityChatBubble({ item, mine, locale, guardLabel }: Props) {
  const signed = useSignedStorageUrls("security-chat", [item.media_url]);
  const mediaUri = item.media_url ? (signed.data?.get(item.media_url) ?? item.media_url) : null;
  ...
  if (type === "image" && mediaUri) {
    return (
      <>
        <ChatImageMessage uri={mediaUri} mine={mine} />
        {imageCaption ? (
  ...
  if (type === "audio" && mediaUri) {
    return (
      <>
        <ChatAudioBubble uri={mediaUri} durationMs={item.media_duration_ms} mine={mine} />
        {audioCaption ? (
          <View style={[styles.bubbleWrap, mine && styles.bubbleWrapMine]}>
            <View style={[styles.bubble, mine && styles.bubbleMine]}>
